...
namespace TC.Agro.Farm.Application.UseCases.Plots.ListAll
{
    ...
    public sealed record ListPlotsQuery : ICachedQuery<PaginatedResponse<ListPlotsResponse>>
    {
        ...
        {
            get => _cacheKey ?? $"ListPlotsQuery-{OwnerId}-{PageNumber}-{PageSize}-{SortBy}-{SortDirection}-{Filter}-{CropType}-{CropTypeCatalogId}-{PropertyId}";
        }
        ...
        {
            _cacheKey = $"ListPlotsQuery-{OwnerId}-{PageNumber}-{PageSize}-{SortBy}-{SortDirection}-{Filter}-{CropType}-{CropTypeCatalogId}-{PropertyId}-{cacheKey}";
        }
    }
}
